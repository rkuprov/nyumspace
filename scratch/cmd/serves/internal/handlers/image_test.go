...
import (
	...
	"github.com/rkuprov/nyumspace/scratch/pkg/foundation/storage"
	"github.com/rkuprov/nyumspace/scratch/pkg/foundation/tests"
...
func TestImageHandlers(t *testing.T) {
	...
	u.RegisterUser(t.Context(), &nyum.UserRegistrationRequest{
		UserRegistrationRequest: nyumpb2.UserRegistrationRequest{
			Username: user.Name,
			Email:    user.Email,
	...
	loginResp, err := u.LoginUser(t.Context(), &nyum.UserLoginRequest{
		UserLoginRequest: nyumpb2.UserLoginRequest{
			Email:    user.Email,
			Password: user.Password,
		},
	})
	...
	homeResp, err := h.CreateHome(t.Context(), &nyum.HomeCreationRequest{
		...
		HomeCreationRequest: nyumpb2.HomeCreationRequest{
			Name:            "Test Home",
